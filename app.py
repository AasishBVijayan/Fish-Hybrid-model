...
if st.button("🔍 Predict Health Status"):
    if uploaded_image is not None:
        ...
        st.write(f"**🖼️ Image Prediction:** {result['image_prediction']} ({result['image_confidence']})")
        st.write(f"**💧 Water Status:** {result['water_status']}")
        st.write(f"**🧠 Final Assessment:** {result['final_assessment']}")

        st.subheader("📉 Specific Water-Based Risks")
        for disease, risk in result['water_based_risks'].items():
            st.write(f"• {disease}: {risk:.0%}")

        st.markdown(
    f"<div style='background-color:#e0ffe0;color:#333;padding:10px;border-radius:8px;'>"
    f"✅ <b>Fish Status:</b> {result['image_prediction']} ({result['image_confidence']})"
    f"</div>", unsafe_allow_html=True)

        import plotly.express as px
        risks_df = pd.DataFrame(list(result['water_based_risks'].items()), columns=['Disease', 'Risk'])
        fig = px.bar(risks_df, x='Disease', y='Risk', color='Risk', range_y=[0,1])
    ...
    file_name="fish_health_report.json")
        ...
        with col1:
            st.image(image, caption="Uploaded Fish", use_container_width=True)
        with col2:
            st.subheader("Diagnosis Summary")
            st.write(f"**Image Prediction:** {result['image_prediction']}")
            st.write(f"**Water Status:** {result['water_status']}")
            st.write(f"**Assessment:** {result['final_assessment']}")
